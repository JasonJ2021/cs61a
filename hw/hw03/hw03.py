...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    return incr_pingpong(n , 1)

def incr_pingpong(n , k):
    if(k == n):
        return 1
    if(k % 8 == 0 or num_eights(k) > 0 ):
        return dec_pingpong(n , k + 1)
    else :
        return 1 + incr_pingpong(n , k + 1)

def dec_pingpong(n , k):
    if( k == n):
        return 0
    if( k % 8 == 0 or num_eights(k) > 0 ):
        return incr_pingpong(n , k + 1)
    else :
        return -1 + dec_pingpong(n , k + 1)
...
def count_coins(change):
    ...
    def partition(change , max_coin):
        ...
        if(max_coin == None):
            return 0
        return partition(change - max_coin , max_coin) + partition(change , descending_coin(max_coin))
