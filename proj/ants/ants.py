...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        for i in range(self.min_range):
            ...
            p = p.entrance
            x += 1
        while(not p.bees and self.min_range <= x <= self.max_range and  not p.is_hive):
            x += 1
            p = p.entrance
        if(x > self.max_range):
            return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if(self.health  - amount <= 0 ):
            damage_all = self.damage + amount
        else :
            damage_all = amount
        if(self.place.bees):
            for bee in self.place.bees[:]:
                bee.reduce_health(damage_all)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        if(self.is_container and self.ant_contained == None and other.is_container == False):
            return True
        return False
        # END Problem 8
