...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def scan(paragraph ):
        list = split(remove_punctuation(paragraph))
        for word in list :
            for word_topic in topic:
                if(lower(word) == lower(word_topic)):
                    return True
        return False
    return scan
    # END PROBLEM 2
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        temp_diff = diff_function(typed_word , word , limit)
        if(word == typed_word):
            return word
        if(temp_diff < min):
            min = temp_diff
...
def feline_flips(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    temp = helper(start , goal , limit , 0 , 0)
    if(temp <= limit ):
        return temp
    else :
        return limit + 1 
    # END PROBLEM 6
def helper(start , goal , limit , k , diff):
    if(diff > limit ) :
        return 0
    if(k >= len(start)):
        return len(goal) - k
    if(k >= len(goal)):
        return len(start) - k  
    if(start[k] == goal[k]):
        return helper(start , goal , limit , k + 1,diff)
    else :
        return 1 + helper(start , goal , limit , k + 1 , 1 + diff)
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    times = [None]*len(times_per_player) # create a list , size of number of players
    for i in range(len(times)):
        times[i] = [None]*(len(times_per_player[i]) - 1)
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        if(ans[min_index] == None):
            ans[min_index] = []
        ans[min_index].append(word_at(match,i))
    return ans
    # END PROBLEM 10
